...
-type request_spec() :: [{
    oas_server:param_name(),
    #{source := param_source(), rules := [oas_server_validation:rule()]}
}].

-type response_spec() :: oas_server_validation:response_spec().
...
-spec authorize_api_key(
    LogicHandler :: module(),
    OperationID  :: oas_server:operation_id(),
    From         :: header | qs_val,
    KeyParam     :: iodata() | atom(),
    Req          :: cowboy_req:req(),
    Context      :: oas_server:request_context()
)->
    {true, oas_server:auth_context(), cowboy_req:req()} |
...
authorize_api_key(LogicHandler, OperationID, From, KeyParam, Req0, Context) ->
    ...
    case ApiKey of
        ...
        _ ->
            Result = oas_server_logic_handler:authorize_api_key(
                LogicHandler,
                OperationID,
                ApiKey,
                Context
...
-spec determine_peer(Req :: cowboy_req:req()) ->
        {ok, Peer :: oas_server:client_peer()} | {error, Reason :: malformed | einval}.
...
-spec populate_request(
    ...
    Spec :: request_spec(),
    Req :: cowboy_req:req(),
    ValidationOpts :: oas_server_validation:validation_opts()
) ->
    {ok,    Populated :: oas_server:object(),       Req :: cowboy_req:req()} |
    {error, Message   :: oas_server:error_reason(), Req :: cowboy_req:req()}.
...
-spec validate_response(
    OperationID :: oas_server:operation_id(),
    Spec     :: response_spec(),
    RespBody :: oas_server:object() | [oas_server:object()] | undefined,
    ValidationOpts :: oas_server_validation:validation_opts()
) ->
    ok | no_return().

validate_response(OperationID, Spec, RespBody, ValidationOpts) ->
    case oas_server_validation:validate_response(OperationID, Spec, RespBody, ValidationOpts) of
        ok ->
            ok;
        {error, Error} ->
...
-spec encode_response(Resp :: oas_server:response()) ->
    Encoded :: response().
...
-spec process_response(
    Status      :: ok | error,
    Result      :: response() | oas_server:error_reason(),
    Req         :: cowboy_req:req(),
    OperationID :: oas_server:operation_id()
) ->
    Req :: cowboy_req:req().
...
populate_request_param(LogicHandler, OperationID, {Name, #{rules := Rules, source := Source}}, Req0, ValidationOpts) ->
    case get_value(Source, Name, Req0) of
        {ok, Value, Req} ->
            case oas_server_validation:prepare_request_param(OperationID, Rules, Name, Value, ValidationOpts) of
                {ok, Result} ->
                    {ok, Name, Result, Req};
                {error, Error}  ->
                    {error, oas_server_logic_handler:map_error(LogicHandler, {validation_error, Error}), Req}
            end;
        {error, Message, Req} ->
            Error = #{
                type => wrong_body,
...
-spec get_value(
    Source :: param_source(),
    Name   :: oas_server:param_name(),
    Req    :: cowboy_req:req()
) ->
    {ok,    Value   :: oas_server:value(),        Req :: cowboy_req:req()} |
    {error, Message :: oas_server:error_reason(), Req :: cowboy_req:req()}.

get_value(body, _Name, Req0) ->
    case oas_server_utils:get_body(Req0) of
        {ok, Body, Req} ->
...
get_value(header, Name, Req) ->
    ...
    {ok, maps:get(oas_server_utils:to_header(Name), Headers, undefined), Req};
get_value(binding, Name, Req) ->
    Bindings = cowboy_req:bindings(Req),
    {ok, maps:get(oas_server_utils:to_binding(Name), Bindings, undefined), Req}.

-spec decode_body(Body :: binary()) ->
    {ok,    Decoded :: oas_server:object() | undefined} |
    {error, Message :: oas_server:error_reason()}.
decode_body(<<>>) ->
    {ok, undefined};
decode_body(Body) ->
    try
