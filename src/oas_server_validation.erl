-module(oas_server_validation).

-export([prepare_request_param/5]).
-export([validate_response/4]).

-type rule()          :: schema | {required, boolean()} | oas_server_param_validator:param_rule().
-type data_type()     :: 'list' | atom().
-type response_spec() :: {data_type(), oas_server:param_name()} | undefined.

-type error() :: #{
    type        := error_type(),
    param_name  => oas_server:param_name(),
    description => oas_server:error_reason()
}.
...
-type validation_opts() :: #{
    schema => oas_server_schema_validator:schema_validation_opts(),
...
-spec validate_response(
    OperationID :: oas_server:operation_id(),
    Spec :: response_spec(),
    Resp :: oas_server:object() | [oas_server:object()] | undefined,
    ValidationOpts :: validation_opts()
) ->
    ok |
    {error, Error :: error()}.
...
-spec validate_param(
    ...
    Prepared :: oas_server:value() | no_return().
...
validate(OperationID, Rule = schema, Name, Value, MsgType, ValidationOpts) ->
    case oas_server_schema_validator:validate(OperationID, Value, Name, MsgType, ValidationOpts) of
        ok ->
            ok;
        {error, Reason} ->
...
validate(OperationID, Rule, Name, Value, MsgType, ValidationOpts) ->
    case oas_server_param_validator:validate(OperationID, Rule, Name, Value, MsgType, ValidationOpts) of
        ok ->
            ok;
        Ok = {ok, _} ->
            Ok;
...
-spec report_validation_error(Rule :: rule(), Param :: oas_server:param_name()) ->
    no_return().
...
-spec report_validation_error(
    Rule        :: rule(),
    Param       :: oas_server:param_name(),
    Description :: oas_server:error_reason() | undefined
) ->
    no_return().
...
-spec map_error(Rule :: rule(), Description :: oas_server:error_reason() | undefined) ->
   Error :: error().
