%% -*- mode: erlang -*-
-module(oas_server_param_validator).

-export([validate/6]).

-type param_base_rule() ::
    oas_server_common_validator:param_rule() |
    {enum, [atom()]} |
...
-type name()     :: oas_server:param_name().
...
validate_with_context(Rule, Value, Meta, ValidationOpts) ->
    case oas_server_custom_validator:validate_param(Rule, Value, Meta, ValidationOpts) of
...
do_validate({enum, Values}, Value, _Meta, _Opts) ->
    try
        FormattedValue = oas_server_utils:binary_to_existing_atom(Value, utf8),
...
do_validate({RuleName, _} = Rule, Value, _Meta, _Opts) when
    ...
    RuleName =:= format
->
    oas_server_common_validator:validate(Rule, Value).
