%% -*- mode: erlang -*-
-module(oas_server_schema_validator).
...
-type msg_type() :: oas_server_validation:msg_type().
...
-type context() ::
    #{
        ...
        msg_type     := msg_type(),
        definition_name     := oas_server:param_name(),
...
-spec validate(
    ...
    Value      :: oas_server:value(),
    DefinitionName    :: oas_server:param_name(),
    MsgType    :: msg_type(),
    ValidationOpts :: validation_opts()
) ->
    ok | {error, Error :: oas_server:error_reason()}.
validate(OperationID, Value, DefinitionName, MsgType, ValidationOpts) ->
    validate(OperationID, Value, DefinitionName, MsgType, oas_server_schema:get(), ValidationOpts).
...
validate(OperationID, Value, DefinitionName, MsgType, Schema, ValidationOpts) ->
    ...
    case jesse:validate_definition(oas_server_utils:to_list(DefinitionName), Schema, Value, Options) of
        {ok, _} ->
            ok;
        {error, [Error]} ->
...
maybe_use_custom_validator(Value, Attr, State) ->
    case oas_server_custom_validator:validate_schema(Attr, Value, State) of
        pass  ->
...
do_check_value(Value, {?FORMAT, Type}, State) when
    ...
    Type =:= <<"binary">>
->
    case oas_server_common_validator:validate({type, erlang:binary_to_atom(Type, utf8)}, Value) of
        error ->
...
do_check_value(Value, {?FORMAT, Format}, State) when
    ...
    Format =:= <<"uri">>
->
    case oas_server_common_validator:validate({format, erlang:binary_to_atom(Format, utf8)}, Value) of
        error ->
            jesse_error:handle_data_invalid(wrong_format, Value, State);
        _ ->
            State
...
validate_child_schema(Value, SchemaName, State) ->
    ...
    BadRef = oas_server_utils:to_list(Ref),
...
map_error_reason({'data_invalid', _Schema, Error, Data, Path0}) ->
    ...
    oas_server_utils:join(".", [Description, Path]).
...
get_error_path(Path0) ->
    Mapper = fun
        (N, Acc) when is_integer(N) ->
            ["[", oas_server_utils:to_binary(N), "]" | Acc];
        (X, Acc) ->
            [$., X | Acc]
    end,
    ...
    Path3 = oas_server_utils:to_binary(Path2),
...
get_error_description({missing_dependency, Dependency0}, _Value) ->
    Dependency = oas_server_utils:to_binary(Dependency0),
    <<"Missing dependency: ", Dependency/binary>>;
get_error_description(missing_required_property, Value) ->
    PropertyName = oas_server_utils:to_binary(Value),
