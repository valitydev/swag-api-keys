%% -*- mode: erlang -*-
-module(oas_client_common_validator).
...
-type result() ::
    ok | {ok, Prepared :: oas_client:value()} | error | {error, Message :: term()}.
...
validate({type, 'integer'}, Value0) ->
    try
        Value = oas_client_utils:to_int(Value0),
        {ok, Value}
    catch
        error:badarg ->
            error
...
validate({type, 'boolean'}, Value) ->
    case oas_client_utils:to_lower(Value) of
        <<"true">> ->  {ok, true};
        <<"false">> -> {ok, false};
        _ -> error
    end;
...
validate({type, 'float'}, Value) ->
    try
        {ok, oas_client_utils:to_float(Value)}
    catch
        error:badarg ->
            error
    end;
...
-spec validate_between(Value :: oas_client:value(), Min :: integer(), Max :: integer()) ->
    ok | no_return().
...
-spec validate_base64(Value :: oas_client:value()) ->
    ok | no_return().
