%% -*- mode: erlang -*-
-module(oas_client_validation).

-export([prepare_request_param/3]).
-export([validate_response/2]).

-type rule()          :: schema | {required, boolean()} | oas_client_param_validator:param_rule().
-type data_type()     :: 'list' | atom().
-type response_spec() :: {data_type(), oas_client:param_name()} | undefined.

-type error() :: #{
    type        := error_type(),
    description => oas_client:error_reason()
}.
...
-spec prepare_request_param(
    Rules :: [rule()],
    Name  :: oas_client:param_name(),
    Value :: oas_client:value()
) ->
    {ok,    Value :: oas_client:value()} |
    {error, Error :: error()}.
...
-spec validate_response(
    Spec :: response_spec(),
    Resp :: oas_client:object() | [oas_client:object()] | undefined
) ->
    ok |
    {error, Error :: error()}.
...
-spec validate_param(
    ...
    Prepared :: oas_client:value() | no_return().
...
validate(Rule = schema, Name, Value, MsgType) ->
    case oas_client_schema_validator:validate(Value, Name, MsgType) of
        ok ->
            ok;
        {error, Reason} ->
...
validate(Rule, Name, Value, _MsgType) ->
    case oas_client_param_validator:validate(Rule, Value) of
        ok ->
            ok;
        Ok = {ok, _} ->
            Ok;
...
-spec report_validation_error(Rule :: rule(), Param :: oas_client:param_name()) ->
    no_return().
...
-spec report_validation_error(
    Rule        :: rule(),
    Param       :: oas_client:param_name(),
    Description :: oas_client:error_reason() | undefined
) ->
    no_return().
...
-spec map_error(Rule :: rule(), Description :: oas_client:error_reason() | undefined) ->
   Error :: error().
