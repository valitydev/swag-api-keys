%% -*- mode: erlang -*-
-module(oas_client_param_validator).

-export([validate/2]).

-type param_base_rule() ::
    oas_client_common_validator:param_rule() |
    {enum, [atom()]} |
...
-spec validate(Rule :: param_rule(), Param :: oas_client:value()) ->
    ok | {ok, Prepared :: oas_client:value()} | error.
...
validate({enum, Values}, Value) ->
    try
        FormattedValue = oas_client_utils:binary_to_existing_atom(Value, utf8),
...
validate({RuleName, _} = Rule, Value) when
    ...
    RuleName =:= format
->
    oas_client_common_validator:validate(Rule, Value).
