%% -*- mode: erlang -*-
-module(oas_client_schema_validator).
...
-spec validate(
    Value   :: oas_client:value(),
    DefName :: oas_client:param_name(),
    MsgType :: msg_type()
) ->
    ok | {error, Error :: oas_client:error_reason()}.
validate(Value, DefName, MsgType) ->
    validate(Value, DefName, MsgType, oas_client_schema:get()).

validate(Value, DefName, MsgType, Schema) ->
    Options = [{validator_opts, #{msg_type => MsgType}} | options()],
    case jesse:validate_definition(oas_client_utils:to_list(DefName), Schema, Value, Options) of
        {ok, _} ->
            ok;
        {error, [Error]} ->
...
validate_child_schema(Value, SchemaName, State) ->
    ...
    BadRef = oas_client_utils:to_list(Ref),
...
validate_format(Value, {?FORMAT, Type}, State) when
    ...
    Type =:= <<"binary">>
->
    case oas_client_common_validator:validate({type, erlang:binary_to_atom(Type, utf8)}, Value) of
        error ->
...
validate_format(Value, {?FORMAT, Format}, State) when
    ...
    Format =:= <<"uri">>
->
    case oas_client_common_validator:validate({format, erlang:binary_to_atom(Format, utf8)}, Value) of
        error ->
            jesse_error:handle_data_invalid(wrong_format, Value, State);
        _ ->
            State
...
map_error_reason({'data_invalid', _Schema, Error, Data, Path0}) ->
    ...
    oas_client_utils:join(".", [Description, Path]).
...
get_error_path(Path0) ->
    Mapper = fun
        (N, Acc) when is_integer(N) ->
            ["[", oas_client_utils:to_binary(N), "]" | Acc];
        (X, Acc) ->
            [$., X | Acc]
    end,
    ...
    Path3 = oas_client_utils:to_binary(Path2),
...
get_error_description({missing_dependency, Dependency0}, _Value) ->
    Dependency = oas_client_utils:to_binary(Dependency0),
    <<"Missing dependency: ", Dependency/binary>>;
get_error_description(missing_required_property, Value) ->
    PropertyName = oas_client_utils:to_binary(Value),
