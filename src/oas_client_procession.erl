...
-type opts()   :: oas_client:transport_opts().
-type object() :: oas_client:object().
-type url()    :: string().
-type method() :: atom().
...
-type request_spec() :: [{
    oas_client:param_name(),
    #{source := param_source(), rules := [oas_client_validation:rule()]}
}].

-type response_spec() :: oas_client_validation:response_spec().
...
-spec process_request(method(), url(), params(), request_spec(), opts()) ->
    {ok, code(), list(), body()} |
    {error, {request_validation_failed,  { oas_client_validation:error(), BadParam :: atom()}}} |
    {error, _Reason}.
...
-spec process_response(response_spec(), body()) ->
    {ok, object()} |
    {error, {response_validation_failed, oas_client_validation:error(), _Response}}.
...
prepare_url(Url, Params, Qs) ->
    oas_client_utils:fill_url(Url, Params, Qs).
...
validate_request_param({Name, #{rules := Rules, source := Source}}, Params) ->
    ...
    case oas_client_validation:prepare_request_param(Rules, Name, Value) of
        {ok, _} ->
            ok;
        {error, Reason} ->
...
get_value(qs_val, Name, Params) ->
    ...
    get_opt(oas_client_utils:to_binary(Name), QueryParams);
get_value(header, Name, Params) ->
    HeaderParams = maps:get(header, Params),
    get_opt(oas_client_utils:to_binary(Name), HeaderParams);
get_value(binding, Name, Params) ->
    BindingParams = maps:get(binding, Params),
    get_opt(oas_client_utils:to_binary(Name), BindingParams).
...
validate_response(Spec, RespBody) ->
    case oas_client_validation:validate_response(Spec, RespBody) of
        ok ->
            ok;
        {error, Error} ->
